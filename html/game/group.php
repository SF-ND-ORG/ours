...
elseif($by=="name")$sql="select name,count(*) as ct from img where tag like '%\r\n$gametag\r\n%' group by name order by count(*) desc";
elseif($by=="date")$sql="select date_format(date,'%Y-%m') as date,count(*) as ct from img where tag like '%\r\n$gametag\r\n%' group by date_format(date,'%Y-%m') order by date_format(date,'%Y-%m') desc";
elseif($by=="class")$sql="select grade,class_type,class,count(*) as ct from img where tag like '%\r\n$gametag\r\n%' group by grade,class_type,class;";
else exit();
  ...
  <div class="card-body">
    <div class="card-title">
      <?php
        ...
        elseif($by=="tag")echo "<h4>一共".sizeof($retval)."个标签(全站)</h4>";
