...
<?php require("../pack/headcard_atc.php")?>
...
{
  $retval=array();
  $sql='select name,count(*) as ct from atc where name_type!=2 group by name order by count(*) desc';
  ...
  $sql='select penname,count(*) as ct from atc where name_type!=0 group by penname order by count(*) desc';
  $retval0=mysqli_query($conn,$sql);
  while($row=mysqli_fetch_assoc($retval0))
  {
    $row["ct"]=(int)$row["ct"];
  ...
  function cmp($a,$b)
  {
    if($a["ct"]==$b["ct"])return 0;
    else return $a["ct"]<$b["ct"]?1:-1;
  }
  uasort($retval,'cmp');

  echo "<div class='card'><div class='card-body'><div class='card-title'>";
  echo "<h4>一共".sizeof($retval)."位同学(蓝姓名青笔名)</h4>";
  echo "</div>";

  foreach($retval as $a)
  {
    if($a["name_type"]=="0") echo "<a href=./conch.php?by=name&exact=1&key=".urlencode($a["name"])."><h5 style='display:inline-block;margin:1%;'><span class='badge badge-pill badge-primary'>".$a["name"]."&nbsp<span class='badge badge-pill badge-light'>".$a['ct']."</span></span></h5></a>";
    else echo "<a href=./conch.php?by=penname&exact=1&key=".urlencode($a["penname"])."><h5 style='display:inline-block;margin:1%;'><span class='badge badge-pill badge-info'>".$a["penname"]."&nbsp<span class='badge badge-pill badge-light'>".$a['ct']."</span></span></h5></a>";
  }
  echo "</div></div>";
...
if($by=="tag")$sql='select tag,count(*) as ct from atc_tag group by tag order by count(*) desc';
elseif($by=="date")$sql="select date_format(date,'%Y-%m') as date,count(*) as ct from atc group by date_format(date,'%Y-%m') order by date_format(date,'%Y-%m') desc";
elseif($by=="class")$sql="select grade,class_type,class,count(*) as ct from atc group by grade,class_type,class;";
else exit();
