    ...
    <form method="get" class="collapse" id="collapse0" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
      <div class="input-group">
        ...
        <select class="form-control" name="by">
            ...
            <option value="atc" <?php echo $by=="atc"?"selected":"";?>>正文</option>
            <option value="name" <?php echo $by=="name"?"selected":"";?>>姓名</option>
            <option value="penname" <?php echo $by=="penname"?"selected":"";?>>笔名</option>
      ...
      <div class="input-group">
        ...
        <select class="form-control" name="order">
            ...
            <option value="name" <?php echo $order=="name"?"selected":"";?>>姓名</option>
            <option value="penname" <?php echo $by=="penname"?"selected":"";?>>笔名</option>
...
if($by=="name")$name_type=" && name_type!=2";
elseif($by=="penname")$name_type=" && name_type!=0";

if($order=="title" || $order=="name")$sql="select * from atc where ".$by." like '".$key."'".$name_type." order by convert(".$order." using gbk)".($up=='1'?" asc":" desc")." limit ".(($page-1)*10).",10";
else $sql="select * from atc where ".$by." like '".$key."'".$name_type." order by ".$order.($up=='1'?" asc":" desc")." limit ".(($page-1)*10).",10";
//echo $sql;
